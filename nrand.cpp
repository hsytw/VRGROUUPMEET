...
// (?嗡?baseline瞍?瘜?)
void random_selection3(map<int, Edge> &person, map<int, Edge> &social, map<int, Edge> &dirEdges, map<int, User> &users, map<int, User> &ans, map<int, User> &ALLUSER);
void random_selection2(map<int, Edge> &person, map<int, Edge> &social, map<int, Edge> &dirEdges, map<int, User> &users, map<int, User> &ans, map<int, User> &ALLUSER);
...
void random_selection3(map<int, Edge> &person, map<int, Edge> &social, map<int, Edge> &dirEdges, map<int, User> &users, map<int, User> &ans, map<int, User> &ALLUSER)
{
    ...
    long long usersize = person.size();
    for (auto u = person.begin(); u != person.end(); ++u)
    {
        /* 產生亂數 */
        for (int i = ans[u->first].connectUser.size(); i < K_capacity; i++)
        {
            int r = rand() % usersize;
            ans[u->first].connectUser.insert(r);

            std::map<int, double>::iterator it;
            it = person[u->first].edges.find(r);
            if (it != person[u->first].edges.end())
            {
                if (checkUinV(ans, u->first, r) == true)
                {
                    // effectiveUtility = (1 - lambdaWeight) * directEdges[u->first].edges[r] + lambdaWeight * (social[u->first].edges[r] + social[r].edges[u->first]) * 1 / 2;
                    totalPersonalutility += (1 - lambdaWeight) * directEdges[u->first].edges[r];
                    totalSocialutility += lambdaWeight * (social[u->first].edges[r] + social[r].edges[u->first]) * 1 / 2;
                }
                else
                {

                    totalPersonalutility += (1 - lambdaWeight) * directEdges[u->first].edges[r];
                    totalSocialutility += 0;
                }
            }
            else
            {
                totalPersonalutility += 0;
                totalSocialutility += 0;
            }
        }
    }
...
Rho_selection max_degree_selection(map<int, Edge> &person, map<int, Edge> &social, map<int, Edge> &dirEdges, map<int, User> &users, map<int, User> &ans, map<int, User> &ALLUSER)
{
    ...
    std::cout << "\nMax Edges: \n";
    std::cout << "UserUID : " << itu->first << " UserVID : " << maxEdgeV << " Single_Selection : " << sflag << endl;
    std::cout << "Max Edges Num = " << maxEdgeNum << endl;
    std::cout << "Effective Utility = " << effectiveUtility << endl;
...
void compare_max_utility(map<int, User> &ans, map<int, Edge> &person, map<int, Edge> &social, map<int, Edge> &dirEdges, map<int, User> &users, string way, map<int, User> &alluser)
{
    ...
    else if (way == "Random")
    {
        ...
        //     std::cout << "user.size() = " << users.size() << endl;
        // }
        random_selection3(person, social, dirEdges, users, ans, alluser);
    }
    else if (way == "Degree")
    {
        while (users.size() > 0)
...
int main(int argc, char *argv[])
{
    ...
    // compare_max_utility(S_star, personalUtility, socialUtility, directEdges, A_theta, "SVC", ALLUSER);
    ...
    // svcflag = 1;
    // outcome.svc_air = calculateNonAirPoint(S_star);
    // outcome.svc_nair = nair_total;
    // svcflag = 0;
    // writeUsers(S_star, 20, "SVC");

    // std::cout << "Personal: " << endl;
    // clear_map();
    // /*read_file(personalUtility, socialUtility, A_theta, directEdges, dataset, true);
    // theta_filter(personalUtility, directEdges, A_theta);
    // calculateEdgeLessThanK(S_star, personalUtility, socialUtility, directEdges, A_theta, "Personal", ALLUSER);*/
    // copyMap();
    // theta_filter(personalUtility, directEdges, A_theta);
    // fastcalculateEdgeLessThanK(S_star, personalUtility, socialUtility, directEdges, A_theta, "SVC", ALLUSER);
    // // addlesskutility(S_star, socialUtility);
    ...
    // writeUsers(S_star, 20, "PER");
    // outcome.per_air = calculateNonAirPoint(S_star);
    // outcome.per_nair = nair_total;
    ...
    // compare_max_utility(S_star, personalUtility, socialUtility, directEdges, A_theta, "Social", ALLUSER);
    ...
    // writeUsers(S_star, 20, "SOC");
    // outcome.soc_air = calculateNonAirPoint(S_star);
    // outcome.soc_nair = nair_total;
    ...
    fastcalculateEdgeLessThanK(S_star, personalUtility, socialUtility, directEdges, A_theta, "SVC", ALLUSER);
    // addlesskutility(S_star, socialUtility);
    //  calculateEdgeLessThanK(S_star, personalUtility, socialUtility, directEdges, A_theta, "Random", ALLUSER);
    auto Random_begin = std::chrono::high_resolution_clock::now();
    compare_max_utility(S_star, personalUtility, socialUtility, directEdges, A_theta, "Random", ALLUSER);
    auto Random_end = std::chrono::high_resolution_clock::now();
    timeoutcome.rand_elapsed_seconds = Random_end - Random_begin;
    std::cout << "Time taken by program is : " << timeoutcome.rand_elapsed_seconds.count() << " sec" << endl;
    outcome.rand.totU = totalUtility;
    outcome.rand.perU = totalPersonalutility;
    outcome.rand.socU = totalSocialutility;
    std::cout << "total utility : " << outcome.rand.totU << endl;
    std::cout << "total p utility : " << totalPersonalutility << endl;
    std::cout << "total s utility : " << totalSocialutility << endl;

    // writeUsers(S_star, 20, "RAND");

    // std::cout << "Max Degree: " << endl;
    // clear_map();
    // /*read_file(personalUtility, socialUtility, A_theta, directEdges, dataset, true);
    // theta_filter(personalUtility, directEdges, A_theta);*/
    // copyMap();
    ...
    // fastcalculateEdgeLessThanK(S_star, personalUtility, socialUtility, directEdges, A_theta, "SVC", ALLUSER);
    // // addlesskutility(S_star, socialUtility);
    // // calculateEdgeLessThanK(S_star, personalUtility, socialUtility, directEdges, A_theta, "Degree", ALLUSER);
    // auto maxD_begin = std::chrono::high_resolution_clock::now();
    // compare_max_utility(S_star, personalUtility, socialUtility, directEdges, A_theta, "Degree", ALLUSER);
    // auto maxD_end = std::chrono::high_resolution_clock::now();
    // timeoutcome.maxD_elapsed_seconds = maxD_end - maxD_begin;
    // std::cout << "Time taken by program is : " << timeoutcome.maxD_elapsed_seconds.count() << " sec" << endl;
    // outcome.maxD.totU = totalUtility;
    // outcome.maxD.perU = totalPersonalutility;
    // outcome.maxD.socU = totalSocialutility;
    // std::cout << "total utility : " << outcome.maxD.totU << endl;
    // std::cout << "total p utility : " << totalPersonalutility << endl;
    // std::cout << "total s utility : " << totalSocialutility << endl;

    // writeUsers(S_star, 20, "MAXD");
    // outcome.maxd_air = calculateNonAirPoint(S_star);
    // outcome.maxd_nair = nair_total;
    ...
    // fastcalculateEdgeLessThanK(S_star, personalUtility, socialUtility, directEdges, A_theta, "SVC", ALLUSER);
    // // addlesskutility(S_star, socialUtility);
    // //  calculateEdgeLessThanK(S_star, personalUtility, socialUtility, directEdges, A_theta, "Avg", ALLUSER);
    // auto avg_begin = std::chrono::high_resolution_clock::now();
    // compare_max_utility(S_star, personalUtility, socialUtility, directEdges, A_theta, "Avg", ALLUSER);
    // auto avg_end = std::chrono::high_resolution_clock::now();
    // timeoutcome.avg_elapsed_seconds = avg_end - avg_begin;
    // std::cout << "Time taken by program is : " << timeoutcome.avg_elapsed_seconds.count() << " sec" << endl;
    // outcome.avg.totU = totalUtility;
    // outcome.avg.perU = totalPersonalutility;
    // outcome.avg.socU = totalSocialutility;
    // std::cout << "total utility : " << outcome.avg.totU << endl;
    // std::cout << "total p utility : " << totalPersonalutility << endl;
    // std::cout << "total s utility : " << totalSocialutility << endl;

    // writeUsers(S_star, 20, "AVG");
    // randflag = 1;
    // outcome.avg_air = calculateNonAirPoint(S_star);
    // outcome.avg_nair = nair_total;
    // randflag = 0;
    ...
    // compare_max_utility(S_star, personalUtility, socialUtility, directEdges, A_theta, "opt", ALLUSER);
    // auto opt_end = std::chrono::high_resolution_clock::now();
    // timeoutcome.opt_elapsed_seconds = opt_end - opt_begin;
    // std::cout << "Time taken by program is : " << timeoutcome.opt_elapsed_seconds.count() << " sec" << endl;
    // outcome.opt.totU = totalUtility;
    // outcome.opt.perU = totalPersonalutility;
    // outcome.opt.socU = totalSocialutility;
    // std::cout << "total utility : " << outcome.opt.totU << endl;
    // std::cout << "total p utility : " << totalPersonalutility << endl;
    // std::cout << "total s utility : " << totalSocialutility << endl;
    // outcome.opt_air = calculateNonAirPoint(S_star);
    // outcome.opt_nair = nair_total;
...
//     std::cout << "start : " << endl;
//     string dataset = "ePFB.csv";
//     //string dataset = "ePFB_c1113_u16.csv";
//     //string dataset = "ePFB_s.csv";
//     read_file(personalUtility, socialUtility, A_theta, directEdges, dataset, true);
//     //watchEdges(personalUtility, 20);
//
//     theta_filter(personalUtility,directEdges,A_theta);
//     //convert_graph(personalUtility, S);
//
//     //watchUsers(A_theta, 20);
//     //std::cout << "\n\n                         Personal                           " << endl << endl;
//     //watchEdges(personalUtility, 20);
//     //watchEdges(directEdges, 20);
//     //std::cout << "\n\n                         Social                           " << endl << endl;
//     watchEdges(socialUtility, 10);
//
//
//     compare_max_utility( S_star, personalUtility, socialUtility, directEdges, A_theta);
//
//
//     //totalCapacity = A_theta.size() * K_capacity;
//     watchUsers( S_star, 350);
//     std::cout << "\nHow many user A: " << A_theta.size() << endl;
//     std::cout << "\nHow many user : " << S_star.size() << endl;
//     std::cout << "total SVACD utility : " << totalUtility << endl;
//
//     // Stop measuring time and calculate the elapsed time
//     auto end = std::chrono::high_resolution_clock::now();
//     //auto elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(end - begin).count();
//     std::chrono::duration<double> elapsed_seconds = end - begin;
//     std::cout << "Time taken by program is : " << elapsed_seconds.count();
//     std::cout << " sec" << endl;
//
//     write_data(dataset, totalUtility, K_capacity, lambdaWeight, personal_theta, elapsed_seconds);
//
//
//
//
...
// ??雿輻??蝷?
//   1. 雿輻 [?寞?蝮賜恣] 閬?嚗憓?蝞∠?瑼?
//   2. 雿輻 [Team Explorer] 閬?嚗???啣?憪??批
//   3. 雿輻 [頛詨] 閬?嚗??梁?撱箄撓?箄??嗡?閮
//   4. 雿輻 [?航炊皜] 閬?嚗炎閬隤?
//   5. ?? [撠?] > [?啣??]嚗遣蝡??撘Ⅳ瑼?嚗??臬?敺 [撠?] > [?啣??暹??]嚗??暹?蝔?蝣潭?獢憓撠?
//   6. 銋?閬?甈⊿??迨撠???隢?敺 [瑼?] > [??] > [撠?]嚗敺??.sln 瑼?
